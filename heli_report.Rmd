...
# from the first column of your data spreadsheet. The test will tell you whether heli_id_long 
# has longer flights than heli_id_short, so put the one you think has longer flights as heli_id_long.
heli_id_long<-"one"
heli_id_short<-"two"
...
```{r}
heli_long_data<-with(helidata, flight_time[heli_id==heli_id_long])
heli_short_data<-with(helidata, flight_time[heli_id==heli_id_short])
```
...
Well, if there is no difference in the two helicopters, then the labels "`r heli_id_long`" and "`r heli_id_short`" are meaningless, and we could randomly shuffle the labels around and get results similar to our actual observations.  If we randomly shuffle the labels around many time and consistently get results different from our observations, then we know that the labels *do* have meaning, and heli "`r heli_id_long`" really does have longer flights on average.
